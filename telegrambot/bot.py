...
GOOGLE_FORM_URL = "GOOGLE_FORM_URL", "https://docs.google.com/forms/d/e/1FAIpQLSeEttFBcOsTwRNrycwvit7CxtUt2xVSJToilxXyE0uaX9hVgA/viewform?usp=dialog"
CALENDLY_URL = "CALENDLY_URL", "https://calendly.com/mabel-choi/30min"
...
# BOT_TOKEN = 'TELEGRAM_BOT_TOKEN', '7618552877:AAENVNLLzNJ0y2ajgOwmnmO3PB1UE-xUTw4')
...
# GEMINI_API_KEY = 'GEMINI_API_KEY', 'AIzaSyB8-_6TBv9Aeg0HeDP2xUWw1lBkRXW_CVk')
# genai.configure(api_key=GEMINI_API_KEY)
# model = genai.GenerativeModel('gemini-2.0-flash')

# async def gemini_analyze(text, context=None):
#     """Use Gemini AI to analyze text"""
#     try:
#         if context:
#             prompt = f"As a job application assistant bot, analyze this message: '{text}'. Context: {context}. Provide a friendly, professional response maintaining the same information but don't add examples about commitment periods. Keep it concise."
#         else:
#             prompt = f"As a job application assistant bot, analyze this message: '{text}'. Provide a friendly, professional response maintaining the same information but don't add examples about commitment periods. Keep it concise."
...
# async def gemini_verify_commitment(text):
#     """Use Gemini AI to help verify commitment period"""
#     try:
#         prompt = f"A job applicant wrote this commitment period: '{text}'. Extract just the numeric value in months. If it mentions years, convert to months (1 year = 12 months). If it mentions weeks, convert to months (4 weeks ≈ 1 month). Return only a number or null if you can't determine a valid commitment period. Don't explain your reasoning."
...
#     except Exception as e:
#         logger.error(f"Error with Gemini AI commitment verification: {e}")
#         return None  # Fallback if Gemini fails
...
#     """Parse commitment period from text and return duration in decimal months"""
#     text_lower = text.lower()
    
#     # Look for patterns like "1.5 months", "0.5 months", "2.3 years", etc.
#     month_patterns = [
#         r'(\d+\.?\d*)\s*months?',
#         r'(\d+\.?\d*)\s*mths?',
#         r'(\d+\.?\d*)\s*mo\b'
...
#             return weeks / 4.33  # Convert to months (approximately)
...
#     raw_message = "Hello! I'm the CPO Pte. Ltd. job application bot. Send me a message with keywords like 'AIRLINE HOUSE' or 'apply' to start your application process."
    
#     # Enhance with Gemini
#     enhanced_message = await gemini_analyze(raw_message)
    
#     await update.message.reply_text(enhanced_message)
...
#         raw_message = "Thank you for contacting CPO Pte. Ltd! Please share with us your resume. If you don't have a resume, please type 'no resume'"
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User is starting the job application process")
        
#         await update.message.reply_text(enhanced_message)
#         return WAITING_FOR_RESUME
#     else:
#         # Don't restart - just give instruction
#         raw_message = "Hello! To start your job application, please include keywords like 'AIRLINE HOUSE', 'apply', 'application', or 'job' in your message."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User sent a message that doesn't contain trigger keywords")
...
#         raw_message = "Thank you for letting us know. Now, please tell us your commitment period in months."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User doesn't have a resume")
...
#                 # Get the user's info for the filename
#                 user_id = update.effective_user.id
#                 username = update.effective_user.username or f"user_{user_id}"
                
#                 # Create a unique filename with timestamp
#                 timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#                 safe_filename = f"{username}_{timestamp}_{file_name}"
...
#                 raw_message = f"Great! I've received your resume ({file_name}). Now, please tell us your commitment period. How long are you willing to commit to this position? (minimum 1 month required)"
                
#                 # Enhance with Gemini
#                 enhanced_message = await gemini_analyze(raw_message, "User submitted resume document")
                
#                 await update.message.reply_text(enhanced_message)
#                 return WAITING_FOR_COMMITMENT
#             else:
#                 raw_message = f"Sorry, the file format '.{file_extension}' is not accepted. Please send your resume as a PDF, JPG, JPEG, or PNG file, or type 'no resume' if you don't have one."
                
#                 # Enhance with Gemini
#                 enhanced_message = await gemini_analyze(raw_message, "User submitted invalid file format")
...
#         # Create a unique filename with timestamp
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#         safe_filename = f"{username}_{timestamp}_resume.jpg"
#         file_path = os.path.join(RESUME_DIR, safe_filename)
...
#         raw_message = "Great! I've received your resume image. Now, please tell us your commitment period in months. (minimum 1 month required)"
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User submitted resume as photo")
...
#     else:
#         raw_message = "Please send your resume as a file (PDF, JPG, JPEG, PNG) or type 'no resume' if you don't have one."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User didn't send valid resume format")
...
#     if not text:
#         raw_message = "Please tell us your commitment period. Minimum requirement: 1 month."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User didn't provide commitment period")
        
#         await update.message.reply_text(enhanced_message)
#         return WAITING_FOR_COMMITMENT
    
#     # First try to parse with our standard function
#     commitment_months = parse_commitment_period(text)
    
#     # If that fails, use Gemini AI to help with verification
#     if commitment_months is None:
#         commitment_months = await gemini_verify_commitment(text)
    
#     if commitment_months is None:
#         raw_message = "I couldn't understand your commitment period. Please specify how many months you can commit to this position."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, "User provided commitment period in invalid format")
        
#         await update.message.reply_text(enhanced_message)
#         return WAITING_FOR_COMMITMENT
    
#     if commitment_months < 1.0:
#         raw_message = f"Sorry, we require a minimum commitment of 1.0 month. You specified: {commitment_months:.1f} months. Please specify at least 1.0 months."
        
#         # Enhance with Gemini
#         enhanced_message = await gemini_analyze(raw_message, f"User provided commitment period less than minimum required: {commitment_months:.1f} months")
        
#         await update.message.reply_text(enhanced_message)
#         return WAITING_FOR_COMMITMENT
    
#     # Success! Send the form link
#     raw_message = f"🎉 Excellent! Thank you for your interest in CPO Pte. Ltd.\n\n✅ Your commitment period: {commitment_months:.1f} months\n✅ This meets our minimum requirement of 1.0 month\n\n📝 Please fill out our application form to complete your application:\n{FORM_LINK}\n\nWe will review your application and get back to you soon. Good luck! 🍀"
    
#     # Enhance with Gemini
#     enhanced_message = await gemini_analyze(raw_message, f"User provided valid commitment period: {commitment_months:.1f} months")
    
#     await update.message.reply_text(enhanced_message)
    
#     # Important: Return END to properly close the conversation
#     return ConversationHandler.END

# async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
#     """Cancel the conversation"""
#     raw_message = "Application process cancelled. Feel free to start again anytime by sending a message with 'apply' or 'AIRLINE HOUSE'."
    
#     # Enhance with Gemini
#     enhanced_message = await gemini_analyze(raw_message, "User cancelled application process")
    
#     await update.message.reply_text(enhanced_message)
#     return ConversationHandler.END

# async def handle_new_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
#     """Handle messages when not in a conversation"""
#     text = update.message.text
    
#     if contains_trigger_keywords(text):
#         # Start the application process by calling the conversation handler directly
#         await handle_message(update, context)
#     else:
#         message = "Hello! To start your job application, please include keywords like 'AIRLINE HOUSE', 'apply', 'application', or 'job' in your message."
