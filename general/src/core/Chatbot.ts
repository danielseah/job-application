...
export class Chatbot {
  private conversationHistories: Map<string, LLMMessage[]> = new Map(); // UserID -> History

  constructor(
    private messagingAdapter: IMessagingPlatformAdapter,
    private llmAdapter: ILLMAdapter,
    private llmOptions?: LLMOptions, // Optional default LLM options
  ) {}
  ...
  private async handleIncomingMessage(
    ...
    console.log(
      `Chatbot: Received message from ${payload.userName || payload.userId}: "${
        payload.text
      }"`,
    );
    ...
    try {
      console.log(
        `Chatbot: Sending to LLM for user ${payload.userId}. History length: ${history.length}`,
      );
      const llmResponseText = await this.llmAdapter.generateResponse(
        history,
        this.llmOptions,
      );
      ...
      if (llmResponseText) {
        ...
        console.log(
          `Chatbot: LLM response for ${payload.userId}: "${llmResponseText}"`,
        );
        ...
        console.warn(
          `Chatbot: LLM returned an empty response for user ${payload.userId}.`,
        );
        await this.messagingAdapter.sendMessage(
          payload.userId,
