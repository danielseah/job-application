...
import {
  ...
  MessagePayload,
} from "./interfaces/IMessagingPlatformAdapter.js";
import { ILLMAdapter, LLMOptions } from "./interfaces/ILLMAdapter.js";
...
async function main() {
  ...
  let messagingAdapter: IMessagingPlatformAdapter;
  ...
  if (geminiApiKey) {
    ...
    console.error(
      "No LLM API key found. Please set OPENAI_API_KEY or GEMINI_API_KEY environment variables.",
    );
    ...
    llmAdapter = {
      generateResponse: async (messages, options) => {
        console.warn(
          "Using Dummy LLM Adapter. No API key found for OpenAI or Gemini.",
        );
  ...
  try {
    await chatbot.start();
    console.log(
      "Chatbot is running. Type 'quit' or 'exit' in the console to stop.",
    );
  } catch (error) {
  ...
  if (messagingAdapter instanceof ConsoleAdapter) {
    const originalOnLine = (messagingAdapter as any).rl?.listeners("line")[0];
    if ((messagingAdapter as any).rl && originalOnLine) {
      (messagingAdapter as any).rl.removeListener("line", originalOnLine);
      (messagingAdapter as any).rl.on("line", async (line: string) => {
        if (line.trim().toLowerCase() === "quit" || line.trim().toLowerCase() === "exit") {
          await shutdown();
        } else {
          // Re-apply the original listener logic
          if ((messagingAdapter as any).onMessage) {
            const payload: MessagePayload = {
              userId: (ConsoleAdapter as any).CONSOLE_USER_ID,
              userName: "Console User",
              text: line.trim(),
              timestamp: new Date(),
            };
            try {
              await (messagingAdapter as any).onMessage(payload);
            } catch (error) {
              console.error("ConsoleAdapter: Error processing message:", error);
            }
          }
          (messagingAdapter as any).rl?.prompt();
        }
      });
    }
  }
